...
ADMIN_TABLE_RULES: dict[str, dict[str, Any]] = {
    ...
    "user_profiles": {
        "editable": {"full_name", "first_name", "last_name", "username", "phone", "age", "gender", "summary_text"},
        "creatable": set(),
        "deletable": True,
    },
    "session_jobs": {
...
def init_db():
    ...
    def _setup_schema(conn):
        ...
        _ensure_column_on_conn(conn, "user_profiles", "first_name", "TEXT")
        _ensure_column_on_conn(conn, "user_profiles", "last_name", "TEXT")
        _ensure_column_on_conn(conn, "user_profiles", "username", "TEXT")
        _ensure_column_on_conn(conn, "user_profiles", "full_name", "TEXT")
        _ensure_column_on_conn(conn, "user_profiles", "phone", "TEXT")
        _ensure_column_on_conn(conn, "user_profiles", "age", "INTEGER")
        _ensure_column_on_conn(conn, "user_profiles", "gender", "TEXT")
        conn.execute(
            text(
...
def get_user_profile(user_id: str | None = None):
    ...
    with _engine_for_user(resolved_user_id).begin() as conn:
        result = conn.execute(
            text(
                """
                SELECT id, user_id, full_name, first_name, last_name, username, phone, age, gender, summary_text, created_at, updated_at
                FROM user_profiles
                WHERE user_id = :user_id
                LIMIT 1
                """
        ...
        conn.execute(
            text(
                ...
                INSERT INTO user_profiles (
                    user_id, full_name, first_name, last_name, username, phone, age, gender, summary_text, created_at, updated_at
                )
                VALUES (
                    :user_id, :full_name, :first_name, :last_name, :username, :phone, :age, :gender, :summary_text, :created_at, :updated_at
                )
                ON CONFLICT(user_id) DO UPDATE SET
                    ...
                    first_name = COALESCE(:first_name, user_profiles.first_name),
                    last_name = COALESCE(:last_name, user_profiles.last_name),
                    username = COALESCE(:username, user_profiles.username),
                    phone = COALESCE(:phone, user_profiles.phone),
                    age = COALESCE(:age, user_profiles.age),
    ...
    with _engine_for_user(resolved_user_id).begin() as conn:
        conn.execute(
            text(
                ...
                VALUES (
                    :user_id, :full_name, :first_name, :last_name, :username, :phone, :age, :gender, '', :created_at, :updated_at
                )
                ON CONFLICT(user_id) DO UPDATE SET
