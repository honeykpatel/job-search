...
def _execute_approved_action(
    ...
    if action_type == "create_helper":
        ...
        created = get_chat_thread(thread_id)
        if not created:
    ...
    if action_type == "rename_helper":
        ...
        helper = get_chat_thread(helper_id)
        ...
        update_chat_thread_title(helper_id, str(params["title"]).strip())
        updated = get_chat_thread(helper_id)
        if not updated:
...
def run_search(payload: SearchRequest) -> dict[str, Any]:
    ...
    if payload.save_results:
        session_id = save_session(job_title, payload.location.strip(), payload.work_style, payload.k)
        save_jobs_for_session(session_id, jobs)
...
def get_sessions(limit: int = Query(default=100, ge=1, le=500)) -> list[dict[str, Any]]:
...
def get_session_jobs_route(session_id: int) -> list[dict[str, Any]]:
    return get_jobs_for_session(session_id)


@app.delete("/api/sessions/{session_id}")
def delete_session_route(session_id: int) -> dict[str, Any]:
...
def get_recent_jobs(limit: int = Query(default=200, ge=1, le=500)) -> list[dict[str, Any]]:
...
@app.get("/api/resumes")
def get_resumes(limit: int = Query(default=100, ge=1, le=500)) -> list[dict[str, Any]]:
    return list_resumes(limit)


@app.get("/api/resumes/latest")
def get_latest_resume_route() -> dict[str, Any] | None:
    row = get_latest_resume()
    if not row:
        return None
...
def get_resume_route(resume_id: int) -> dict[str, Any]:
    resume = get_resume(resume_id)
    if not resume:
        raise HTTPException(status_code=404, detail="Resume not found")
    return resume
...
async def upload_resume(file: UploadFile = File(...)) -> dict[str, Any]:
    ...
    try:
        ...
        resume_id = save_resume(file.filename, text)
    ...
    return {
        ...
        "text": text,
        "created_at": get_resume(resume_id)["created_at"],
    }


@app.delete("/api/resumes/{resume_id}")
def delete_resume_route(resume_id: int) -> dict[str, bool]:
...
def get_applications(
    ...
    return list_applications(status=status, limit=limit)


@app.put("/api/applications/{job_id}")
def update_application(job_id: str, payload: ApplicationUpdateRequest) -> dict[str, Any]:
    try:
...
def delete_application_route(job_id: str) -> dict[str, bool]:
...
def get_profile() -> dict[str, Any]:
    return get_user_profile() or {"summary_text": ""}
...
def update_profile(payload: ProfileUpdateRequest) -> dict[str, Any]:
    return save_user_profile(payload.summary_text)


@app.get("/api/threads")
def get_threads(limit: int = Query(default=100, ge=1, le=500)) -> list[dict[str, Any]]:
    return list_chat_threads(limit)


@app.post("/api/threads/general")
def create_general_thread() -> dict[str, Any]:
    thread_id = get_or_create_general_thread()
...
def create_job_thread(payload: CreateThreadRequest) -> dict[str, Any]:
...
def get_thread(thread_id: int) -> dict[str, Any]:
    ...
    job = get_job(thread["job_id"]) if thread.get("job_id") else None
    resume = get_resume(thread["resume_id"]) if thread.get("resume_id") else None
    return {
        **thread,
        "job": job,
        "resume": resume,
...
def clear_thread(thread_id: int) -> dict[str, bool]:
    clear_chat_thread(thread_id)
    touch_chat_thread(thread_id)
    return {"ok": True}


@app.delete("/api/threads/{thread_id}")
def delete_thread(thread_id: int) -> dict[str, bool]:
    delete_chat_thread(thread_id)
    return {"ok": True}


@app.post("/api/threads/{thread_id}/messages")
def send_message(thread_id: int, payload: ChatRequest) -> dict[str, Any]:
    thread = get_chat_thread(thread_id)
    if not thread:
        raise HTTPException(status_code=404, detail="Thread not found")
    ...
    add_chat_message(thread_id, "user", content)
    existing_messages = get_chat_messages(thread_id)
    thread_type = thread.get("thread_type", "job")
    thread_job = get_job(thread["job_id"]) if thread.get("job_id") else None
    thread_resume = get_resume(thread["resume_id"]) if thread.get("resume_id") else None
    ...
    thread_context = build_thread_context(
        ...
        pipeline_summary=build_pipeline_summary() if thread_type == "general" else None,
            ...
            for message in new_messages:
                if message.type == "tool":
                    ...
                    if isinstance(parsed, dict) and parsed.get("needs_confirmation"):
                        ...
                        continue
                    add_chat_message(thread_id, "tool", message.content)
    ...
    if last_assistant:
        add_chat_message(thread_id, "assistant", last_assistant)
...
def approve_thread_action(thread_id: int, payload: ApprovalActionRequest) -> dict[str, Any]:
    thread = get_chat_thread(thread_id)
    if not thread:
        raise HTTPException(status_code=404, detail="Thread not found")
